...
func (h *BadgeHandler) Public(c *gin.Context) {
	farmer, score, anchor, err := h.service.ForFarmer(c.Param("farmer_id"))
	if err != nil {
		c.JSON(404, gin.H{"error": gin.H{"code": "SCORE_NOT_YET_SHAREABLE", "message": "score is not yet shareable"}})
		return
	}
	c.JSON(200, gin.H{"farmer_name": farmer.Name, "overall_score": score.Grade, "ledger_tx_id": anchor.TxID, "chain": anchor.Chain, "verified_at": anchor.AnchoredAt})
}
